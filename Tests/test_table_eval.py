...
class TestTableEvaluator(unittest.TestCase):
    def test_is_match(self):
        ...
        for gt in empty:
            for pred in empty:
                ...
                self.assertEqual(is_match(pred, gt), 1)
            for pred_num in number:
                ...
                for pred_list in pred_valid:
                    for pred in pred_list:
                        ...
                        self.assertEqual(is_match(pred, gt), 0)
            for pred in invalid:
            ...
            for pred_num in number:
                ...
                for sign_level_pred, pred_list in enumerate(pred_valid):
                    for pred in pred_list:
                        ...
                        self.assertEqual(is_match(pred, gt), 0)
            for j, pred in enumerate(invalid):
                # If both are invalid, should not count it at all
                self.assertEqual(is_match(pred, gt), 1 if i == j else 0)
    ...
    def test_eval_table_simple(self):
        ...
        self.assertEqual(perf["Table accuracy"], 1)
    ...
    def test_eval_table_multi_panel(self):
        ...
        perf = evaluate_table(pred_df, gt_df)
        self.assertAlmostEqual(perf["Table accuracy"], 1-(2+19)/252)
