...
class TestPlotEvaluatorCalPerf(TestPlotEvaluator):
    ...
    def test_cal_perf(self):
        ...
        curves_in_subplot = {
            "1": [
                {"x": ([], []), "y": (pred_value, gt_value), "err": (pred_err, gt_err) }
            ]
        }
        ans = {
    ...
    def test_cal_perf_multi_curve(self):
        ...
        curves_in_subplot = {
            "1": [
                {"x": (pred_x[0], gt_x[0]), "y": (pred_value[0], gt_value[0]), "err": ([], [])},
                {"x": (pred_x[1], gt_x[1]), "y": (pred_value[1], gt_value[1]), "err": ([], [])}
            ]
        }
        first_curve = {
    ...
    def test_cal_perf_multi_subplot_multi_curve(self):
        ...
        pred_value = [[85, 80, 75, 70, 65], [25, 22, 20, 18, 15], [-0.15, -0.1, -0.05, 0.0, 0.1, 0.05]]
        gt_x = [[1995, 2000, 2005, 2010, 2012], [1995, 2000, 2005, 2010, 2012]]
        gt_value = [[84.06889128094726, 79.1173304628633, 70.18299246501616, 61.14101184068892, 57.91173304628633], [25.188374596340154, 21.95909580193758, 15.931108719052745, 13.024757804090413, 12.917115177610327], [-0.1957403651115618, -0.3600405679513184, -0.061866125760649, -0.1369168356997971, 0.0740365111561865, 0.0801217038539553]]
        pred_err = [0.1, 0.08, 0.12, 0.07, 0.09, 0.06]
        gt_err = [0.2251521298174442, 0.2718052738336714, 0.1176470588235294, 0.1440162271805274, 0.2393509127789046, 0.2900608519269776]
        ...
        curves_in_subplot = {
            ...
            "2": [
                {"x": ([], []), "y": (pred_value[2], gt_value[2]), "err": (pred_err, gt_err)}
            ]
        }
...
class TestPlotEvaluatorPairing(TestPlotEvaluator):
    ...
    def DISABLE_test_hist_range(self):
    ...
    def DISABLE_test_multi_panel(self):
    ...
    def DISABLE_test_type1_subplot_float(self):
